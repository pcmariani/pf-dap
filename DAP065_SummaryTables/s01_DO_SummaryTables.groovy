...
for( int i = 0; i < dataContext.getDataCount(); i++ ) {
    ...
    // println multiSelectParam
    if (multiSelectParam) {
      def item = multiSelectParam[0]
      def keysArr = []

      if (item.PivotConfig == "GroupBy") {
        keysArr = activeGroupByConfigsArr.RowKey.collect { it.toUpperCase().split(DBIFS) }.transpose()
      }
      else if (item.PivotConfig == "PivotOn") {
        keysArr = pivotedDataConfigsArr.ColumnKey.collect { it.toUpperCase().split(DBIFS) }.transpose()
      }

      if (keysArr) {
        keysArr.each {
          ArrayList valuesArr = multiSelectParam.Value[0].toUpperCase().split(/\s*,\s*/)
          // if (!valuesArr.disjoint(it)) {
          if (valuesArr.clone().sort() == it.clone().sort()) {
            sortKeysArr = it
          }
        }
      }
